...
# AV plots for each continent
for (j in 1:ncol(C)) {
  Cj = C[, j]
  fitCj_1235 = mylm(Cj, cbind(X1, X2, X3, D))

  plot(fitCj_1235$sres, fit1235$sres,
       main = paste("AV plot for Continent:", colnames(C)[j]),
       xlab = paste("sres for", colnames(C)[j], "~ (X1, X2, X3, D)"),
       ylab = "sres for Y ~ (X1, X2, X3, D)")
  abline(h = 0, v = 0, lwd = 2)

  mCj = cor(fitCj_1235$sres, fit1235$sres) * sd(fit1235$sres) / sd(fitCj_1235$sres)
  bCj = mean(fit1235$sres) - mCj * mean(fitCj_1235$sres)

  abline(bCj, mCj, col = "red", lwd = 3)
}

# AV plot for Least Developed
# Handle D whether it's a matrix or vector
if (is.matrix(D) && ncol(D) > 1) {
  # Multiple columns - loop through each
  for (j in 1:ncol(D)) {
    Dj = D[, j]
  ...
  # Single column (vector or 1-col matrix) - plot once
  plot(fit5_1234$sres, fit1234$sres,
       main = "AV plot for Least Developed",
       xlab = "sres for Least Developed ~ (X1, X2, X3, C)",
       ylab = "sres for Y ~ (X1, X2, X3, C)")
  abline(0, 0, lwd = 2)
  abline(mean(fit1234$sres) - mD * mean(fit5_1234$sres), mD, col = "red", lwd = 3)
}
...
```{r lasso}
# Prepare design matrix (without intercept - lasso function adds it)
...
# Find optimal lambda by minimizing AIC
