...
```{r}
slope_CO2 = partial_slope(X, Y, 2, c(1,3,4,5,6))

slope_expend = partial_slope(X, Y, 3, c(1,2,4,5,6))

slope_obesity = partial_slope(X, Y, 4, c(1,2,3,5,6))

slope_continent = partial_slope(X, Y, 5, c(1,2,3,4,6))

slope_develop = partial_slope(X, Y, 6, c(1,2,3,4,5))
```
...
fit12345 = mylm(Y, cbind(X1,X2,X3,C,D))
fit1234= mylm(Y, cbind(X1,X2,X3,C))
fit1235= mylm(Y, cbind(X1,X2,X3,D))
...
fit4_1235= mylm(C, cbind(X1,X2,X3,D))
fit5_1234= mylm(D, cbind(X1,X2,X3,C))
...
for (j in 1:ncol(C)) {
  Cj = C[, j]
  fitCj_1235 = mylm(Cj, cbind(X1, X2, X3, D))
  mC[j] = cor(fitCj_1235$sres, fit1235$sres) *
    sd(fit1235$sres) / sd(fitCj_1235$sres)
}

mD = cor(fit5_1234$sres, fit1234$sres) * sd(fit1234$sres) / sd(fit5_1234$sres)
for (j in 1:ncol(C)) {
  Cj = C[, j] 
  fitCj_1235 = mylm(Cj, cbind(X1, X2, X3, D))
  
  plot(fit4_1235$sres, fit1235$sres,
       main = paste("AV plot for Continent", colnames(C)[j]),
       xlab = paste("sres for", colnames(C)[j], "~ (X1, X2, X3, D)"),
       ylab = "sres for y ~ (X1, X2, X3, D)")
  abline(h = 0, v = 0, lwd = 2)
       
  mCj = cor(fit4_1235$sres, fit1235$sres) * sd(fit1235$sres) / sd(fit4_1235$sres)
  bCj = mean(fit1235$sres) - mCj * mean(fit4_1235$sres)

  abline(bCj, mCj, col = "red", lwd = 3)
}

for (j in 1:ncol(D)) {
  Dj = D[, j]
  fitDj_1234 = mylm(Dj, cbind(X1, X2, X3, C))
  
  plot(fitDj_1234$sres, fit1234$sres,
       main = paste("AV plot for Least Developed:", colnames(D)[j]),
       xlab = paste("sres for", colnames(D)[j], "~ (X1, X2, X3, C)"),
       ylab = "sres for Y ~ (X1, X2, X3, C)")
  abline(0,  0, lwd = 2)
  
  mDj = cor(fitDj_1234$sres, fit1234$sres) * sd(fit1234$sres) / sd(fitDj_1234$sres)
  bDj = mean(fit1234$sres) - mDj * mean(fitDj_1234$sres)
  
  abline(bDj, mDj, col = "red", lwd = 3)
}
...
```{r lasso}
lambda_seq = seq(0, 100, by=0.5)
lasso_results = lambda(Xk, Y, lambda_seq)

optimal_lambda = lasso_results$optimal_lambda
optimal_coeffs = lasso_results$optimal_result$coefficients

cat("Optimal Lambda:", optimal_lambda, "\n")
cat("Optimal Coefficients:\n")
print(optimal_coeffs)

plot(lambda_seq, lasso_results$aic_vals, type="l", col="blue", lwd=2,
     xlab="Lambda", ylab="AIC", main="Lasso: AIC vs Lambda")
abline(v=optimal_lambda, col="red", lty=2)
```

The Lasso regression analysis identifies the optimal penalization parameter (lambda) that balances model fit and complexity. The optimal lambda value minimizes the AIC, selecting only the most important predictors while shrinking less important coefficients toward zero. This approach provides a data-driven method for variable selection that complements the drop-one and AV plot analyses.
...
### Function 2: mylm - Linear Regression
...
### Function 3: my_drop1_ss - Drop-One Sum-of-Squares Analysis
...
### Function 4: seq_ss - Sequential Sum-of-Squares
...
### Function 5: mlr - Multiple Linear Regression
...
### Function 6: lasso - Lasso Regression with Penalty Parameter
...
### Function 7: lambda - Lasso Lambda Optimization
