...
The optimal penalty parameter was λ = 1258.925, at which point all nine predictors remained in the model with non-zero coefficients. This indicates that, even with L1 regularization, all predictors contribute meaningfully to explaining life expectancy.
...
To select the optimal lambda value, we need to balance stability (measured by the condition number $\kappa$) with prediction accuracy (measured by RMSE). I chose a condition number threshold of $\kappa < 50$ as the critical stability criterion. This threshold is statistically justified because:
...
```{r ridge_testpoints}
# Use the same three test points as before
spain = which(data$Country == "Spain" & data$Year == 2015)
nigeria = which(data$Country == "Nigeria" & data$Year == 2000)
iran = which(data$Country == "Iran" & data$Year == 2009)

# OLS predictions (using beta_hat from earlier)
pred_ols_spain = sum(X[spain, ] * beta_hat)
pred_ols_nigeria = sum(X[nigeria, ] * beta_hat)
pred_ols_iran = sum(X[iran, ] * beta_hat)

# Ridge predictions (using beta_ridge from optimal lambda)
pred_ridge_spain = sum(X[spain, ] * beta_ridge)
pred_ridge_nigeria = sum(X[nigeria, ] * beta_ridge)
pred_ridge_iran = sum(X[iran, ] * beta_ridge)

# Comparison table
comparison = data.frame(
  Country = c("Spain", "Nigeria", "Iran"),
  Year = c(2015, 2000, 2009),
  Actual = round(c(Y[spain], Y[nigeria], Y[iran]), 2),
  OLS = round(c(pred_ols_spain, pred_ols_nigeria, pred_ols_iran), 2),
  Ridge = round(c(pred_ridge_spain, pred_ridge_nigeria, pred_ridge_iran), 2),
  Difference = round(c(pred_ols_spain - pred_ridge_spain,
                       pred_ols_nigeria - pred_ridge_nigeria,
                       pred_ols_iran - pred_ridge_iran), 2)
)

comparison
```

For Spain in 2015, the OLS model predicted 78.69 years while Ridge predicted 78.71 years, a difference of only 0.02 years. For Nigeria in 2000, OLS predicted 57.86 years compared to Ridge's 57.84 years (difference: 0.02 years). Iran in 2009 showed the same pattern, with OLS at 73.20 years and Ridge at 73.19 years (difference: 0.01 years).

These extremely small differences (all under 0.03 years) suggest that multicollinearity was not severely affecting the OLS estimates in this dataset. This aligns with our earlier VIF analysis, which showed all VIF values well below concerning thresholds. The ridge model's value here is not in dramatically different predictions, but in providing more stable coefficient estimates that would be less sensitive to small changes in the data or to predictions made outside the observed range.

From an application standpoint, these subtle differences would not lead to different policy interpretations for these specific countries. Both models agree that Spain has high life expectancy, Nigeria has substantially lower life expectancy, and Iran falls in the middle range. However, the ridge model's improved numerical stability would be more valuable in scenarios where:
- Predictions are made for data points far from the training data
- The model is updated frequently with new data
- Coefficient stability is critical for understanding long-term trends
...
This project took the exploratory work from Project 1 and shifted it toward prediction. Instead of just describing how life expectancy varies across continents or years, I built models to actually predict it—using multiple linear regression, ridge regression, and logistic regression—to see which environmental, health, and structural factors matter most.
...
I chose a condition number threshold of κ < 50 because it strikes a good balance: it's low enough to ensure numerical stability without over-penalizing the coefficients. Anything above 50 starts to show signs of ill-conditioning, where small data changes cause big swings in estimates.
